...
int optionRegister(coap_resource_t **resource , char* temp_str){
	...
	if (upper_status && type_size > 0 && data_size > 0){
		...
		if(status){
			coap_add_attr(*resource, coap_new_str_const(opt_type, strlen(opt_type)),
				coap_new_str_const(opt_data, strlen(opt_data)),
...
int pathRegister(coap_resource_t *old_resource, coap_resource_t **new_resource , char* temp_str){
	...
	if (calPathSize(temp_str) > 0){
		int total_size = old_resource->uri_path->length + calPathSize(temp_str) + 2; // supposed to be 1, but for safety reason I add 1 more byte
		...
		if(status){
			...
			char* parent_path = malloc(sizeof(char) * (old_resource->uri_path->length + 2));
			snprintf(parent_path,old_resource->uri_path->length+1, "%s", old_resource->uri_path->s);
			sprintf(abs_path,"%s/%s", parent_path, rel_path);
			free(parent_path);
			*new_resource = coap_resource_init(
					coap_new_str_const(abs_path, strlen(abs_path)), COAP_RESOURCE_FLAGS_RELEASE_URI); 
		}
		free(rel_path);
		return status;
	}
...
int calculateResourceLF(coap_resource_t* resource){
	if(resource != NULL){
		...
		lf_size += resource->uri_path->length + 3;
